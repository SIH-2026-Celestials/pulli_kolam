...
Last updated: session 23 (real-kolam GRAPH-quality preprocessing
experiment — NEGATIVE result, see Session 23 below; NOT committed yet,
uncommitted working-tree changes). Extended `engine/canonicalize.py`
...
integrated. M4.1 status unchanged: **PARTIAL**
(`docs/M4_1_ML_COMPLETION_REPORT.md`). Session 22 (canonicalization
false-positive experiment — also negative, 5 variants, no-dot FP stayed
100% for all).
(M4.1 ML completion campaign — gating experiment — see below).
